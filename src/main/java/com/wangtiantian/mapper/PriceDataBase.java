...
public class PriceDataBase {
    ...
    public void price_cityData(ArrayList<CityData> dataList) {
        int batchSize = 100;
        T_Config_Price cityDataDao = new T_Config_Price(2, chooseDataBase, 0);
    ...
    public void dealerData(ArrayList<DealerData> dataList) {
         T_Config_Price dealerDao = new T_Config_Price(2, chooseDataBase, 1);
        int batchSize = 100;
        for (int i = 0; i < dataList.size(); i += batchSize) {
            int end = Math.min(i + batchSize, dataList.size());
...
//            System.out.println("经销商数据入库操作");
    ...
    public void saleModData(ArrayList<SaleModData> dataList) {
         T_Config_Price saleModDao = new T_Config_Price(2, chooseDataBase, 2);
        int batchSize = 100;
        for (int i = 0; i < dataList.size(); i += batchSize) {
            int end = Math.min(i + batchSize, dataList.size());
    ...
    public void carPriceData(ArrayList<CarPrice> dataList) {
         T_Config_Price carPriceDao = new T_Config_Price(2, chooseDataBase, 4);
        int batchSize = 100;
        for (int i = 0; i < dataList.size(); i += batchSize) {
            int end = Math.min(i + batchSize, dataList.size());
