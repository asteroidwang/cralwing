...
//    }
    ...
    public void update_修改已下载的车辆价格信息的状态(){
        String sql ="update "+tableName+" set C_IsFinish = 1 where concat(C_DealerId,'_',C_ModelId) in (select distinct concat(C_DealerId,'_',C_ModelId) from T_ConfirmCarPriceFile_20241010)";
        method_i_d_u(sql);
    }
