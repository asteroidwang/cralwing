...
public class T_Config_KouBei extends T_Config_Father {
    ...
    public ArrayList<Object> method_分页查询未下载的有二级评论的数据10000条每次(int begin) {
        String sql = "SELECT * FROM " + tableName + " where C_IsFinish = 0  and C_replyId !='-' ORDER BY C_ID OFFSET " + begin + " ROWS FETCH NEXT 10000 ROWS ONLY";
        return method_有条件的查询(sql);
    }
    public int get_回复表中数据总量(){
        String sql ="select count(*) from "+tableName+ "  where C_IsFinish = 0  and C_replyId !='-' ";
